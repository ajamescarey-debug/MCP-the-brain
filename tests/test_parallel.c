...
TEST(lsp_resolve_qualified_static_call_normalizes_colons) {
    ...
    const CBMResolvedCall *hit = cbm_pipeline_find_lsp_resolution(&arr, &call);
    ASSERT(hit != NULL);
    ...
    const CBMResolvedCall *bare_hit = cbm_pipeline_find_lsp_resolution(&arr, &bare);
...
TEST(lsp_resolve_misattribution_is_bounded) {
    ...
    const CBMResolvedCall *other_hit = cbm_pipeline_find_lsp_resolution(&arr, &other);
    ...
    ASSERT(cbm_pipeline_find_lsp_resolution(&arr, &absent) == NULL);
    PASS();
}
